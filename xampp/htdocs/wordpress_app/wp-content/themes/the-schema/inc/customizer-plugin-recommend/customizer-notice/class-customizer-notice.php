<?php
/**
 * The Schema Customizer Notification System.
 *
 * @package The_Schema
 */
 ...
 */
class The_Schema_Customizer_Notice {
	...
	/**
	 * The single instance of The_Schema_Customizer_Notice
	 *
	 * @var The_Schema_Customizer_Notice $instance The The_Schema_Customizer_Notice instance.
	 */
	...
	/**
	 * The Main The_Schema_Customizer_Notice instance.
	 *
	 * We make sure that only one instance of The_Schema_Customizer_Notice exists in the memory at one time.
	 *
	 * @param array $config The configuration array.
	 */
	public static function init( $config ) {
		if ( ! isset( self::$instance ) && ! ( self::$instance instanceof The_Schema_Customizer_Notice ) ) {
			self::$instance = new The_Schema_Customizer_Notice;
	...
	public function setup_config() {
		...
		$the_schema_recommended_plugins = array();
		
		if ( isset( $this->recommended_plugins ) ) {
			$the_schema_recommended_plugins = $this->recommended_plugins;
		}
	...
	public function scripts_for_customizer() {
		...
		wp_enqueue_style( 'the-schema-customizer-notice', get_template_directory_uri() . '/inc/customizer-plugin-recommend/customizer-notice/css/customizer-notice.css', null );
		...
		wp_enqueue_script( 'the-schema-customizer-notice', get_template_directory_uri() . '/inc/customizer-plugin-recommend/customizer-notice/js/customizer-notice.js', array( 'jquery', 'customize-controls' ), THE_SCHEMA_THEME_VERSION, true );
		wp_localize_script(
			'the-schema-customizer-notice', 
            'customizer_notice_data', 
            array(
				'ajaxurl'            => admin_url( 'admin-ajax.php' ),
				'template_directory' => get_template_directory_uri(),
				'base_path'          => admin_url(),
				'activating_string'  => __( 'Activating', 'the-schema' ),
			)
		);
	}
	...
	public function customize_register_notice( $wp_customize ) {
		/**
		 * Include the The_Schema_Customizer_Notice_Section class.
		 */
		...
		$wp_customize->register_section_type( 'The_Schema_Customizer_Notice_Section' );

		$wp_customize->add_section(
			new The_Schema_Customizer_Notice_Section(
				$wp_customize,
	...
	public function dismiss_recommended_plugins_callback() {
		...
		if ( ! empty( $action_id ) ) {
			...
			update_option( 'the_schema_show_recommended_plugins', $show_recommended_plugins );
		}
