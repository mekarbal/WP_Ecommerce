<?php
/**
 * Envo Business Customizer Notification System.
 *
 ...
 */
class Envo_Business_Customizer_Notice {
	...
	/**
	 * The single instance of Envo_Business_Customizer_Notice
	 *
	 * @var Envo_Business_Customizer_Notice $instance The Envo_Business_Customizer_Notice instance.
	 */
	...
	/**
	 * The Main Envo_Business_Customizer_Notice instance.
	 *
	 * We make sure that only one instance of Envo_Business_Customizer_Notice exists in the memory at one time.
	 *
	 * @param array $config The configuration array.
	 */
	public static function init( $config ) {
		if ( ! isset( self::$instance ) && ! ( self::$instance instanceof Envo_Business_Customizer_Notice ) ) {
			self::$instance = new Envo_Business_Customizer_Notice;
	...
	public function setup_config() {
		...
		global $envo_business_recommended_plugins;
		...
		$envo_business_recommended_plugins = array();
		
		if ( isset( $this->recommended_plugins ) ) {
			$envo_business_recommended_plugins = $this->recommended_plugins;
		}
	...
	public function scripts_for_customizer() {
		...
		wp_enqueue_style( 'envo-business-customizer-notice', get_template_directory_uri() . '/lib/customizer-plugin-recommend/customizer-notice/css/customizer-notice.css', array() );
		...
		wp_enqueue_script( 'envo-business-customizer-notice', get_template_directory_uri() . '/lib/customizer-plugin-recommend/customizer-notice/js/customizer-notice.js', array( 'customize-controls' ) );
		wp_localize_script(
			'envo-business-customizer-notice', 'customizer_notice_data', array(
				'ajaxurl'                  => admin_url( 'admin-ajax.php' ),
				'template_directory'       => get_template_directory_uri(),
				'base_path'                => admin_url(),
				'activating_string'        => __( 'Activating', 'envo-business' ),
			)
		);
	...
	public function customize_register_notice( $wp_customize ) {
		...
		/**
		 * Include the Envo_Business_Customizer_Notice_Section class.
		 */
		...
		$wp_customize->register_section_type( 'Envo_Business_Customizer_Notice_Section' );

		$wp_customize->add_section(
			new Envo_Business_Customizer_Notice_Section(
				$wp_customize,
	...
	public function dismiss_recommended_plugins_callback() {
		...
		if ( ! empty( $action_id ) ) {
			...
			$show_recommended_plugins = get_option( 'envo_business_show_recommended_plugins' );
			...
			update_option( 'envo_business_show_recommended_plugins', $show_recommended_plugins );
		}
